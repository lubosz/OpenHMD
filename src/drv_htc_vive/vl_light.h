...
void print_sample_group (const vl_light_sample_group& g) {
    printf("channel %c (len %ld, samples %zu): skip %d, sweep %c epoch %d\n",
...
std::map<uint32_t, vl_angles> collect_readings(char station, const std::vector<vl_light_sample_group>& sweeps) {
    // Collect all readings into a nice data structure
    // x and y angles, and a timestamp (x sweep epoch)
    // array R(sensor_id + 1).x, .y, .t

    std::map<uint32_t, vl_angles> R;
    ...
    for (int i = 1; i < maxseq; i++) {
        ...
        for (uint32_t sweep_i = 0; sweep_i < x_sweeps.size(); sweep_i++) {
            ...
            for (uint32_t s = 0; s < 32; s++) {
                ...
                if (!R.count( s ))
                    R.insert(std::pair<uint32_t, vl_angles>(s, vl_angles()));
...
std::map<uint32_t, vl_angles>
vl_light_extract_angles_from_samples(char channel,
                                 vl_lighthouse_samples *raw_light_samples) {
    vl_lighthouse_samples sanitized_light_samples =
            filter_reports(*raw_light_samples, &is_sample_valid);
    std::vector<vl_light_sample_group> pulses;
    std::vector<vl_light_sample_group> sweeps;
    std::tie(sweeps, pulses) = process_lighthouse_samples(sanitized_light_samples);
    return collect_readings(channel, sweeps);
}

const float ticks_per_degree = 2222.22;
...
perspective_n_point(char channel, vl_lighthouse_samples *raw_light_samples,
    ...
    std::map<uint32_t, vl_angles> sensor_angles =
            vl_light_extract_angles_from_samples(channel, raw_light_samples);
    ...
    float fx = 0.5;
    int w = angle_range_v[1] - angle_range_v[0];
    int h = angle_range_h[1] - angle_range_h[0];
    ...
    for (auto sensor_angle : sensor_angles) {
        ...
        cv::Point3f configSensor = config_sensor_positions.at(sensor_angle.first);

        vl_angles angles = sensor_angle.second;

        // cv::Point2f angles_cv = cv::Point2f(angles.x[0]/1000, angles.y[0]/1000);

        cv::Point2f angles_cv = cv::Point2f(
                ((float) angles.x[0] / ticks_per_degree) - angle_range_v[0],
                ((float) angles.y[0] / ticks_per_degree) - angle_range_h[0]);
        /*
        printf("%d [%f, %f] <> [%f, %f, %f]\n",
               angles.first,
               a_cv.x,
    ...
    cv::Mat object_points_cv = cv::Mat(object_points);
    cv::Mat image_points_cv = cv::Mat(image_points);
...
vl_light_std_angles_from_samples(char channel,
                                     vl_lighthouse_samples *raw_light_samples) {

    std::map<uint32_t, vl_angles> sensor_angles =
            vl_light_extract_angles_from_samples(channel, raw_light_samples);

    printf("got %ld readings\n", sensor_angles.size());

//    cv::Mat rvec, tvec;
//    if (!raw_light_samples->empty())
//        std::tie(tvec, rvec) = try_pnp(raw_light_samples, config_sensor_positions);

    std::map<uint32_t, std::vector<uint32_t>> angle_result;
    ...
    for (auto sensor_angle : sensor_angles) {
        ...
        std::vector<uint32_t> angles_vector = {angles.x[0], angles.y[0]};

        angle_result[sensor_angle.first] = angles_vector;
    }
    return angle_result;
}
